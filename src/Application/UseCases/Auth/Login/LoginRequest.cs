...
public class LoginRequestHandler : IResultRequestHandler<LoginRequest, LoginResponse>
{
    ...
    public async Task<Result<LoginResponse>> Handle(LoginRequest request, CancellationToken cancellationToken)
    {
        ...
        var ownerLocations = user.LocationUsers.Select(ol => (ol.Location.Name, ol.Location.PlaceId));

        return new LoginResponse(token, user.Name, user.Email, ownerLocations).ToResult();
    }
}

public record LoginResponse(string Token, string Name, string Email, IEnumerable<(string Name, string PlaceId)> OwnerLocations);